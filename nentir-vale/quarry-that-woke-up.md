...
- **Permanent stops all sever its command:** cut its conduit-vein (the Scene 7 junction); drive it past the **ward-line** or up toward the surface, where the Voice dies and it goes **inert**; or drop it in a terminal hazard (collapse, flood, the arc-forge). This is the finale's lesson taught early — *beat the Voice, not the bodies.*
- **Arc:** a gate it holds at the breach (Scene 4) → a stalking dread → the permanent showdown at the conduit junction (Scene 7), which also cripples the boss. **Telegraph the outs** (Renzo half-saw it; Ssvara names the fix) so the terror drives problem-solving, not despair.
...
- **Role:** Missing father; found at the throne (Scene 8), half-made into **the Centurion** — the living commander the Haruspex must install so it can issue new orders (and extend the front to the surface). Rescuing him and denying the devil its command are the same act.
...
- **Visual:** Fifties, big-knuckled and barrel-chested, grey stubble; now pallid, brass-sheened eyes and a slow parade-ground stiffness, a Turathi bronze gorget fused to his collarbone. **Casting:** Pierfrancesco Favino, or John Turturro.
...
- **Secret:** He heard the Voice's first orders and *ran while the others were taken*; he can describe the war-room, the reanimated dead, and Donato being marched to the throne. He'll downplay his own flight unless pressed gently.
- **Voice:** *"They didn't die, that's the thing — it kept them. It called them by rank. It made Donato its 'Centurion,' like it had waited centuries for him."*
...
- **Secret:** Her clutch has served the broadcast orders for generations as a garrison-cult; she is the first to doubt, and terrified her own kind will call her a traitor. She knows the ward-line the Lictor won't cross, the murder-hole layout, and the secret the doubters live by: **the Voice weakens with distance and dies at the surface.**
...
- **Encounter:** **10 kobolds** (≈2 pts, deliberately easy). They fight to a *badly-remembered* Turathi drill — front rank slings, rear rank reloads — and bark clipped Infernal parade-commands they only half-understand (*"Hold! Volley! Re-form!"*), disengaging in order rather than dying to the last. One wears a scrap of Turathi bronze as a rank-badge. **Clue:** this is real military doctrine, poorly copied — something *trained* them, and they answer to a "Voice."
...
- **Encounter — the Lictor** *(see Order of Battle):* the party arrives as the bronze executioner drags a bound, screaming cutter toward the conduit-forge to be killed and raised. It is nigh-unkillable and **holds the threshold — it will not cross the ward-line** (standing order). Outs: rescue the captive and fall back past the ward-line (it won't follow); bloody it to force a **withdrawal** and slip by; or bypass it entirely (stealth/another route). Brute force *fails loudly* — that's the lesson. Bypassing the ward (DC 14 Arcana/Thieves' Tools) is quieter than triggering it (alarm — wakes Scene 5 early).
...
### 7. The Muster *(the Lictor climax — stop the unstoppable, cripple the boss)*
- **Purpose:** Pay off the dungeon-long dread: here the party *permanently* stops the Lictor — not with damage but by severing its command — and cripples the devil on the way to the throne.
- **Setting:** The **conduit gallery** — a rising corridor of glowing infernal circuitry linking War-Room to Throne, two blast-door chokepoints, and a central **conduit junction** that can be *cut* (DC 14 tools/Arcana). Cutting it drops the Lictor **inert**, staggers the devil (halves its HP / ends *call the reserve* in Scene 8), *and* blocks the free-it ending. A cold thread of *outside air* pulls uphill here — luring the Lictor toward the surface also stops it.
- **At stake:** Get through with the Lictor dealt with. Sensing the threat to its last war-machine, the devil finally **commits the Lictor to the kill** and stops hoarding it. The objective is to reach and cut the junction (or lure the Lictor to the surface, or drop it in the arc-forge); a straight fight is still a losing race.
- **Encounter — the Lictor, unleashed** + a thin screen of **kobolds and anchoring dead** at the blast-doors (the dead plug the doorways, the kobolds funnel). A *puzzle under pressure*: solve the Lictor through the environment while the screen buys the devil time. If the party earned Ssvara's aid (Scene 6), her clutch pulls the screen and turns a brutal race into a winnable one.
- **Ways in / ways out:** From the Doubting Clutch (Scene 6) or straight from the War-Room (Scene 5). Out: the Command Throne (Scene 8) — the boss already crippled if the conduit was cut here.
...
- **The three paths:**
  - **CON (intended clever win).** The Haruspex is *bound to obey command authority* — it cannot act, or stand down, without a commander's order. It is lucid on procedure and insane on reality, so "you lost" never works; a **correctly-formed order from proper authority** does (exact requirements in **The stand-down protocol**, GM Notes). Two keys fit the lock: **(a)** produce or forge superior authority — the dig-journal, Turathi bronze, a cease-fire order in Infernal (DC 15 Deception/Forgery) — and use correct protocol; or **(b) free Donato and have him, as the Haruspex's own installed Centurion, order the stand-down** — the devil is bound to obey its commander (this fuses the rescue and the clever win). Run it as a social skill challenge — **3 successes before 2 failures** on DC 13–15 (History/Persuasion/Deception/Arcana; advantage for real Turathi tokens or for wielding Donato's authority). Success: it stands down, releases Donato, powers the bunker to sleep — the relief from duty it has craved for centuries. Failure: it brands them infiltrators — roll to FIGHT with the garrison already engaged.
...
2. **The counter-sign.** The Bael Turath watchword from the Scene 5 interrogation — proves the order is legitimate, not enemy trickery.
...
4. **A token of relief.** The ritual closes on a physical token pressed to the throne — the **Sigil of the Ninth Reach** (see Vault loot) or, lacking it, Donato's own hand as Centurion.

**Running it:** treat each element the party brings as an automatic success (or advantage) on the CON skill challenge (3 successes before 2 failures). A party with **authority + counter-sign + form + token** succeeds outright, no rolls — doing it *correctly* is the whole point. Missing pieces are covered by rolls at DC 13–15; **missing authority cannot be rolled past.**
...
### Vault loot
The "something big and old" Donato found. It sits in a strongroom off the throne chamber (Scene 8), recovered after the finale. Most of it is **heavy bronze wealth** — the thing that actually saves the Aldabri family, and what Serica's split is fighting over (her deal governs the party's cut).
...
- **Sigil of the Ninth Reach** *(uncommon wondrous; also the stand-down token)* — a bronze command-seal. 1/day, as an action, utter a one-word command in Infernal that a **bound Bael Turath creature** (a construct or lesser devil — the Lictor, a lemure) must obey (Wis save DC 13 negates). **No power over the Haruspex itself.**
